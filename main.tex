...
&=\frac{1}{4 \pi}\frac{e^{-m_\pi^2}}{m_\pi r}\left[\sqrt{\frac{8\pi}{15}}Y_2(\hat{r})\cdot\sigmatwo\left(1+\frac{3}{m_\pi r}+\frac{3}{(m_\pi r)^2}\right)+\frac{\sigmadot}{3}\right]
\end{split}
\end{equation}
...
Similar to Novato \emph{et al} %\cite{LOVATO}
, we introduce the following notation for the dimensionless radial functions $w(\alpha,\beta,\ell; m r)$, defined by
...
which implies that
\begin{align*}
w(\alpha,\beta,\ell; m r) = \frac{1}{m^{3+\alpha-2\beta} } \frac{2}{\pi} \int dq \, q^2 \, \frac{q^\alpha}{(q^2+m^2)^\beta} j_\ell(q r)
\end{align*}
with $z \equiv m r$. Below, we tabulate this integral for selected values of $\alpha, \beta, \ell$. The first function we recover, for the case of $(\alpha, \beta, \ell ) = (0,1,0)$, is nothing but a Yukawa potential. To further demonstrate the use of this integral, we now derive the familiar tensor force resulting from one pion exchange. This amplitude is related to the integral
...
$$ (\sigma^{(1)} \cdot \mathbf{q}) ( \sigma^{(2)} \cdot \mathbf{q} )= q^2 \left( [\sigma^{(1)} \otimes \sigma^{(2)}]_2 \cdot \bar{Y}_2(\hat{q}) + \tfrac{1}{3} \sigma^{(1)} \cdot \sigma^{(2)}  \bar{Y}_0(\hat{q}) \right),$$
which then gives
$$ V_\pi( \mathbf{r}) \propto \left(\frac{m^3}{4 \pi}\right) \left\{ - [\sigma^{(1)} \otimes \sigma^{(2)}]_2 \cdot \bar{Y}_2(\hat{r}) w(2,1,2; z) + \tfrac{1}{3} \sigma^{(1)} \cdot \sigma^{(2)} w(2,1,0; z ) \right\},  $$
or, plugging in the $w$'s and noting that the tensor operator $S_{12}$ is conventionally defined
$$ 3 [\sigma^{(1)} \otimes \sigma^{(2)}]_2 \cdot \bar{Y}_2(\hat{r}) \equiv S_{12}, $$
we have
$$ V_\pi( \mathbf{r}) \propto -\left(\frac{m^3}{12 \pi}\right) \left\{ \frac{e^{-z}}{z} \left[ S_{12}\left(1+\frac{3}{z}+ \frac{3}{z^2} \right) +  \sigma^{(1)} \cdot \sigma^{(2)} \right] - \frac{4\pi }{m^3}  \sigma^{(1)} \cdot \sigma^{(2)} \delta^3(\mathbf{r}) \right\}, $$
...
2 & 1 & 0 & $ -\frac{\displaystyle e^{-z}}{\displaystyle z} + 4\pi \delta^3(\vec{r})/m^3 $ \\
...
4 & 2 & 0 & $  \frac{\displaystyle e^{-z}}{\displaystyle z} (\tfrac{z}{2} - 2 ) + 4 \pi \delta^3( \vec{r} ) / m^3 $ \\
